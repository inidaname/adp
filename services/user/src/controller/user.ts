...
export async function getUserById(
  ...
  try {
    ...
    const findUser = await userModel
      .findById(id)
      .and([{ $not: { userStatus: 'deactivated' } }])
      .lean()
      .select('-__v')
      .exec();
...
export async function updateUser(
  ...
  try {
    ...
    return res
      .status(201)
      .json({ data: updateU, message: `Updated successfully` });
  } catch (error) {
    return res.status(error.status || 500).json({ message: error.message });
  }
}
...
export async function deactivateUser(
  ...
  try {
    ...
    const deactivate = await userModel
      .findByIdAndUpdate(id, { userStatus: `deactivate` }, { new: true })
      .and([{ $not: { userStatus: 'deactivated' } }])
      .lean()
      .select('-__v')
      .exec();
    ...
    return res
      .status(201)
      .json({ data: deactivate, message: `User deleted successfully` });
  } catch (error) {
    return res.status(error.status || 500).json({ message: error.message });
  }
}
...
export async function requestPassword(
  ...
  try {
    ...
    if (!id) {
      throw {
        status: 404,
        message: `User with provided details does not exist`
      };
    }

    const findEmail = [
      await userModel
        .findOne({ $or: [{ email: id }, { phoneNumber: id }] })
        .and([{ $not: { userStatus: 'deactivated' } }]),
      await userModel
        .findById(id)
        .and([{ $not: { userStatus: 'deactivated' } }])
    ...
    Promise.all(findEmail).then(
      value => {
        value.map(async (user, i, a) => {
          if (user) {
            const token = await createToken(user);
            const createReset = await resetModel.create({
              userId: user._id,
              token
            });
            if (createReset) {
              ...
              return res
                .status(200)
                .json({ message: `Email for change of password sent`, token });
            }
          }
        });
      },
...
export async function checkReset(
  ...
  try {
    ...
    const check: ResetInterface = await resetModel
      .findOneAndUpdate(
        { $and: [{ requestStatus: 'active' }, { token }] },
        { requestStatus: 'touched' },
        { new: true }
      )
      .lean()
      .select('-__v')
...
export async function confirmReset(
  ...
  try {
    ...
    const check: ResetInterface = await resetModel
      .findOneAndUpdate(
        { $and: [{ requestStatus: 'touched' }, { token }] },
        { requestStatus: 'done' },
        { new: true }
      )
      .lean()
    ...
    return res
      .status(200)
      .json({ message: `Reset found`, changed: check, data: changePass });
  } catch (error) {
    return res.status(error.status || 500).json({ message: error.message });
  }
}
